...
- **Feature modules** in `src/modules/{clients,projects,time}/`. **Clients is fully built** (`types.ts`, `schema.ts` zod validation, `queries.ts`, `actions.ts`, `display.ts`, `components/`) and is the reference implementation; Projects and Time are still `types.ts`+`queries.ts` stubs. New modules follow the same shape; routed pages live under `(app)/`. Server actions accept `unknown` and validate with zod at the boundary.
